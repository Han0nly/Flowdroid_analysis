...
class MobSF_result:
    ...
    def analyse_strings(self):
        self.str_result = {}
        with open('strings.md','w') as stringfile:
            for item in self.content:
                self.str_result[item['file_name']] = []
                stringfile.write(item['file_name']+'\n'+"```\n")
                if item['strings']:
                    for found in item['strings']:
                        found = found.lower()
                        if ("password" in found) or ("passwd" in found) or ("key" in found) or ("_id" in found) or ("token" in found) or ("secret"in found) or ("license"in found):
                            self.str_result[item['file_name']].append(found)
                            stringfile.write(found + '\n')
                stringfile.write("```\n")
    ...
    def analyse_certificate(self):
        ...
        for item in self.content:
            if item['certificate_analysis']:
                for found in item['certificate_analysis']['certificate_findings']:
                    if found[0] == 'bad':
                        self.print_result[item['file_name']] = ['Only Provide v1 certificate']
                        if ("certbad_" + found[1]) in self.cert_result.keys():
                            self.cert_result["certbad_" + found[1]] = self.cert_result["certbad_" + found[1]] + 1
                        else:
                            self.cert_result["certbad_" +found[1]] = 1
                    else:
                        if ("cert_" + found[1]) in self.cert_result.keys():
                            self.cert_result["cert_" + found[1]] = self.cert_result["cert_" + found[1]] + 1
                        else:
                            self.cert_result["cert_" +found[1]] = 1
        ...
        for item in self.content:
            if item['code_analysis']:
                for weakness in item['code_analysis'].keys():
                    if item['code_analysis'][weakness]['metadata']['severity'] == "high":
...
def main():
    ...
    workbook.save(filename='./new_result1.xlsx')
